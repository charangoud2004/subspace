...
def _get_seed_domain(argv: list[str]) -> str:
    """Return the seed domain from CLI args or an interactive prompt."""
    if len(argv) > 1:
        return argv[1].strip()
...
def _print_checkpoint(
    ...
    """Print the mandatory safety summary and return true only for exact y confirmation."""
    print("\n============================================")
...
def _abort(message: str, exit_code: int = 1) -> None:
    """Print a message and exit the process with the provided status code."""
    print(message)
    raise SystemExit(exit_code)


def _warn_zero_emails() -> None:
    """Ask the user whether to abort when no emails were resolved."""
...
def run(seed_domain: str) -> dict[str, int]:
    """Run all outreach stages sequentially for one seed domain and return send counts."""
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and run the outreach pipeline."""
